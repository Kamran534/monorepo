...
export interface CategoryProps {
  /**
   * Category repository instance for data access
   */
  repository: CategoryRepository;
  /**
   * Function to get connection state (optional)
   * Returns an object with dataSource: 'server' | 'local'
   */
  getConnectionState?: () => Promise<{ dataSource: 'server' | 'local' }> | { dataSource: 'server' | 'local' };
}

export function Category({ repository, getConnectionState }: CategoryProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check connection state if provided
        let isOnline = true;
    ...
    fetchData();
  }, [dispatch, repository, cacheAge, getConnectionState]);
  ...
  return (
    <div className="h-full w-full p-8 overflow-auto" style={{ backgroundColor: 'var(--color-bg-primary)' }}>
      ...
      {parentCategories.map((parentCategory) => {
        ...
        const childCategories: NonNullable<CategoryType['childCategories']> = parentCategory.childCategories || [];
